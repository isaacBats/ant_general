...
class UpdraftPlus_Admin {
	...
	private function create_backup_dir() {
		...
		if (!$wp_filesystem->is_dir($default_backup_dir) && !$wp_filesystem->mkdir($default_backup_dir, 0)) {
			$wperr = new WP_Error;
		...
		if ($wp_filesystem->is_dir($default_backup_dir)) {
			...
			@$wp_filesystem->chmod($default_backup_dir, 0);// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
			if (UpdraftPlus_Filesystem_Functions::really_is_writable($updraft_dir)) return true;
