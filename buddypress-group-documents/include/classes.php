...
class BP_Group_Documents {
	...
	public static function get_list_by_group( $group_id, $sort=0, $order=0, $start=0, $items=0 ){
		global $wpdb, $bp;
		...
		--$start;	
 		...
 		$result = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$bp->group_documents->table_name} WHERE group_id = %d ORDER BY $sort $order LIMIT %d, %d", $group_id, $start, $items ), ARRAY_A );

		return $result;
	}

	public static function get_list_for_newest_widget($num, $group_filter=0 ) {
		global $wpdb,$bp;

		if( $group_filter && BP_GROUP_DOCUMENTS_WIDGET_GROUP_FILTER ) {
			$result = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$bp->group_documents->table_name} WHERE group_id = %d ORDER BY created_ts DESC LIMIT %d", $group_filter, $num), ARRAY_A );
	...
	public static function get_list_for_popular_widget($num, $group_filter=0 ) {
		global $wpdb,$bp;

		if( $group_filter && BP_GROUP_DOCUMENTS_WIDGET_GROUP_FILTER ) {
			$result = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$bp->group_documents->table_name} WHERE group_id = %d ORDER BY download_count DESC LIMIT %d", $group_filter, $num), ARRAY_A );
